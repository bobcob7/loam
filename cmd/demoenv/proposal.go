...
// SAME work branch is in the queue after its first accept, gone once a
// conflicting target advance resets it to draft, back once it has been
// caught up and re-approved, and gone for good once its PR merges. Only
// the absence half proves the queue's predicate is evaluating anything.
func runCheckProposals(args []string, stdin io.Reader, stdout io.Writer) error {
	...
	selectName := fs.String("select-name", "", "address the one proposal for this work branch (required by -state/-pr-url/-approver/-round)")
	...
	wantsRow := *wantState != "" || *wantPRURL != "" || *wantApprover != "" || *wantRound != unsetCount
	switch {
	case *selectName == "" && wantsRow:
		failures = append(failures, "-state/-pr-url/-approver/-round address one proposal; pass -select-name to name it")
	case *selectName != "":
		failures = append(failures, proposalRowFailures(doc, *selectName, *wantState, *wantPRURL, *wantApprover, *wantRound)...)
	}
...
func proposalRowFailures(doc proposalsDoc, name, wantState, wantPRURL, wantApprover string, wantRound int) []string {
	for _, p := range doc.Proposals {
