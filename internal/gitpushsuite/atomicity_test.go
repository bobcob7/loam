...
func TestAtomicity_OneBadRefRejectsTheWholePush_GoodRefNeverLands(t *testing.T) {
	...
	require.Empty(t, mirrorRefSHA(t, env.mirrorDir, "refs/heads/loam-reserved/wb-good"), "wb-good must not exist on the mirror before this push")
	beforeMain := mirrorRefSHA(t, env.mirrorDir, "refs/heads/main")
	require.NotEmpty(t, beforeMain, "main must already exist (seeded)")
	out, err := pushRefs(t, clonePath, "HEAD:refs/heads/loam-reserved/wb-good", "HEAD:refs/heads/main")
	...
	assert.Empty(t, mirrorRefSHA(t, env.mirrorDir, "refs/heads/loam-reserved/wb-good"), "the individually-good ref must NOT have landed on the mirror: pre-receive semantics reject the WHOLE push")
