...
func TestRejection5_ForcePush_RejectedByGitOwnDenyNonFastForwards_NotTheHook(t *testing.T) {
	...
	firstSHA := mirrorRefSHA(t, env.mirrorDir, "refs/heads/loam-reserved/wb-good")
	require.NotEmpty(t, firstSHA)
	runGit(t, clonePath, "commit", "--quiet", "--amend", "-m", "diverged, rewriting history")
	out, err = pushRefs(t, clonePath, "--force", "HEAD:refs/heads/loam-reserved/wb-good")
	...
	assert.Equal(t, firstSHA, mirrorRefSHA(t, env.mirrorDir, "refs/heads/loam-reserved/wb-good"), "the mirror's ref must not have moved to the diverged commit")
...
func TestRejection6_Delete_RejectedByGitOwnDenyDeletes_NotTheHook(t *testing.T) {
	...
	out, err := pushRef(t, clonePath, "refs/heads/loam-reserved/wb-good2")
	require.NoError(t, err, "the initial create must succeed: %s", out)
	firstSHA := mirrorRefSHA(t, env.mirrorDir, "refs/heads/loam-reserved/wb-good2")
	require.NotEmpty(t, firstSHA)
	out, err = pushRefs(t, clonePath, ":refs/heads/loam-reserved/wb-good2")
	...
	assert.Equal(t, firstSHA, mirrorRefSHA(t, env.mirrorDir, "refs/heads/loam-reserved/wb-good2"), "the branch must still exist on the mirror, at its original commit -- never deleted")
