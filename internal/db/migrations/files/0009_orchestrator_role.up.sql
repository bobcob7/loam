...
Identity. The identifier is <name>-<id>-<role>, built from three separate variables: LOAM_AGENT_NAME is the bare name, LOAM_AGENT_ID the bare id, LOAM_AGENT_ROLE the role. Putting the whole identifier into NAME writes doubled names into permanent review records and cannot be fixed afterwards. Those three have a built-in default -- this orchestrator identity -- used when all of them are left unset; they default together, so setting any one makes all three required and a forgotten role is an error rather than a role nobody chose. Author and reviewer must be different identities, because roles gate capabilities -- a reviewer holds git.clone but not git.push and genuinely cannot push.
