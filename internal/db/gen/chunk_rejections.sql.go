...
const deleteChunkRejections = `-- name: DeleteChunkRejections :exec
DELETE FROM chunk_rejections
WHERE repo_id = $1 AND target_branch = $2 AND file = ANY($3::text[])
`

type DeleteChunkRejectionsParams struct {
	RepoID       pgtype.UUID
	TargetBranch string
	Column3      []string
}
...
func (q *Queries) DeleteChunkRejections(ctx context.Context, arg DeleteChunkRejectionsParams) error {
	_, err := q.db.Exec(ctx, deleteChunkRejections, arg.RepoID, arg.TargetBranch, arg.Column3)
	return err
}

const deleteChunkRejectionsForRepoBranch = `-- name: DeleteChunkRejectionsForRepoBranch :exec
DELETE FROM chunk_rejections
WHERE repo_id = $1 AND target_branch = $2
`

type DeleteChunkRejectionsForRepoBranchParams struct {
	RepoID       pgtype.UUID
	TargetBranch string
}
...
func (q *Queries) DeleteChunkRejectionsForRepoBranch(ctx context.Context, arg DeleteChunkRejectionsForRepoBranchParams) error {
	_, err := q.db.Exec(ctx, deleteChunkRejectionsForRepoBranch, arg.RepoID, arg.TargetBranch)
	return err
}

const listChunkRejections = `-- name: ListChunkRejections :many
SELECT repo_id, target_branch, file, attempts, state, chunks_state, sqlstate, error, job_id, rejected_ref, first_rejected_at, last_rejected_at FROM chunk_rejections
WHERE repo_id = $1 AND target_branch = $2
ORDER BY file
`

type ListChunkRejectionsParams struct {
	RepoID       pgtype.UUID
	TargetBranch string
}
...
func (q *Queries) ListChunkRejections(ctx context.Context, arg ListChunkRejectionsParams) ([]ChunkRejection, error) {
	rows, err := q.db.Query(ctx, listChunkRejections, arg.RepoID, arg.TargetBranch)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ChunkRejection
	for rows.Next() {
		var i ChunkRejection
		if err := rows.Scan(
			&i.RepoID,
			&i.TargetBranch,
			&i.File,
...
const recordChunkRejection = `-- name: RecordChunkRejection :exec
...
type RecordChunkRejectionParams struct {
	RepoID       pgtype.UUID
	TargetBranch string
	File         string
	Column4      int32
...
func (q *Queries) RecordChunkRejection(ctx context.Context, arg RecordChunkRejectionParams) error {
	_, err := q.db.Exec(ctx, recordChunkRejection,
		arg.RepoID,
		arg.TargetBranch,
		arg.File,
		arg.Column4,
