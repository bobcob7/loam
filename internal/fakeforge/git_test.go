...
func TestGitInfoRefsReceivePackDeniedForReadOnlyToken(t *testing.T) {
	...
	ctx := t.Context()
	require.NoError(t, srv.SeedRepoFiles(ctx, "acme/widgets", map[string][]byte{"a": []byte("b")}, SeedOptions{}))
	req, err := http.NewRequest(http.MethodGet, ts.URL+"/git/acme/widgets.git/info/refs?service=git-receive-pack", nil)
	require.NoError(t, err)
...
func TestGitAuthRejectsMissingBasicAuth(t *testing.T) {
	...
	ctx := t.Context()
	require.NoError(t, srv.SeedRepoFiles(ctx, "acme/widgets", map[string][]byte{"a": []byte("b")}, SeedOptions{}))
	resp, err := http.Get(ts.URL + "/git/acme/widgets.git/info/refs?service=git-upload-pack")
	require.NoError(t, err)
	defer resp.Body.Close()
...
func TestGitAuthRejectsBadToken(t *testing.T) {
	...
	ctx := t.Context()
	require.NoError(t, srv.SeedRepoFiles(ctx, "acme/widgets", map[string][]byte{"a": []byte("b")}, SeedOptions{}))
	req, err := http.NewRequest(http.MethodGet, ts.URL+"/git/acme/widgets.git/info/refs?service=git-upload-pack", nil)
	require.NoError(t, err)
